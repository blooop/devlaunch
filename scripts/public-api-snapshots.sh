...
#   scripts/public-api-snapshots.sh --print-nightly
...
# The nightly that renders them, pinned for the same reason. rustdoc renders
# the signatures cargo-public-api prints, so a new nightly moves rows with no
# crate change: 2026-09-24 started printing `clone(&self) -> Self` where
# 2026-09-17 printed the full path, and every snapshot went red on a PR that
# had touched none of those types. `cargo +<toolchain> public-api` is the one
# way the tool takes a toolchain; left to itself it falls back to a bare
# `nightly`, because `rust/rust-toolchain.toml` pins a stable. Bump it together
# with regenerated files, as PIN is.
...
core="$(cargo "+$NIGHTLY" public-api -p devlaunch-core "${FLAGS[@]}")"
pattern="$(promised_row_pattern)"
...
cargo "+$NIGHTLY" public-api -p devlaunch-runner "${FLAGS[@]}" >"$staging/$RUNNER_FILE"
