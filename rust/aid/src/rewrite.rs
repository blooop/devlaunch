...
mod tests {
    ...
    fn a_verb_line_asks_dl_for_the_verb_and_starts_no_agent() {
        ...
        assert_eq!(
            build_dl_args(
                &parsed(&["owner/repo@fix/x", "review this pr", "--rm", "--force"]),
                None
            )
    ...
    fn a_verb_flag_before_the_spec_is_the_same_request() {
        ...
        assert_eq!(
            build_dl_args(&parsed(&["--rm", "owner/repo"]), None).expect("a verb line"),
            ["owner/repo", "--rm"]
        );
    ...
    fn a_herdr_pane_is_named_on_the_agents_own_command_line() {
        ...
        assert_eq!(
            ...
            Some(
                "IS_SANDBOX=1 HERDR_ENV=1 HERDR_PANE_ID=w1:p2 \
    ...
    fn autorm_lands_between_the_spec_and_the_agent_command() {
        ...
        let built = build_dl_args(&parsed(&["owner/repo", "fix it", "--autorm"]), None)
    ...
    fn autorm_before_the_spec_is_the_same_request() {
        ...
        let built = build_dl_args(&parsed(&["--autorm", "owner/repo", "fix it"]), None)
    ...
    fn a_verb_beside_autorm_keeps_the_verbs_own_reading() {
        ...
        assert_eq!(chosen.spec_options, ["--autorm"]);
        assert_eq!(
            build_dl_args(&chosen, None).expect("a verb line"),
