...
/// wrappers come and go, so `grep -rn "impl Runner for"` is the enumeration
/// rather than a number written here. Two shapes are worth recognising before
/// reading one:
///
...
///   docker, whose real daemon would be the developer's own — and hands
///   everything else, git above all, to real processes. That is how a test drives
///   a real repository against a workspace that never existed;
///   `flows::listing`'s `FakeDevpodRealGit` is the one named after the pattern.
