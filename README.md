...
A CLI for [devpod](https://devpod.sh) with tab completion and a built-in fuzzy selector.
...
a devcontainer built from that repo, on that branch. Every `repo@branch` you name gets a container
of its own, so switching branches is switching containers rather than rebuilding one.
...
to the container that is already there, in about a second. So the line you used to create the
...
beside the first. The branch does not have to exist yet; `dl` makes one if it cannot find it.
...
`aid` is `dl` with a coding agent started for you, on the same clone and in the same container `dl`
would have given you for that spec. Name the repo and it asks for the prompt *while the container
boots*, so the minute the container takes and the minute you take writing the prompt are the same
minute. Press Enter and you are in the container, with the agent working in the checkout. That is a
third container; the feature branch from step 2 is still running beside it. Put the prompt on the
line instead, `aid blooop/devlaunch@feature/json-flag add a --json flag`, and it skips the question.
...
- **Fuzzy selection.** Called with no arguments, or with a verb and no workspace, `dl` opens a
  built-in fuzzy selector. Nothing to install, and no `fzf` on `PATH` to find.
- **Tab completion** for workspaces, GitHub repos in `owner/repo` form, and paths. The completion
  cache answers in about 3ms, against roughly 700ms without it.
- **GitHub shorthand.** `owner/repo` expands to `github.com/owner/repo`, and `owner/repo@branch`
  names a branch.
- **One round-trip per question.** Every `devpod` call costs about 0.45s, far more than `dl` itself,
  so a command reads the workspace list at most once. Everything a container needs on the way in,
  naming it and then the tools probe, rides one setup pass, so an interactive `dl <ws>` and a
  one-shot `dl <ws> -- <cmd>` cost the same trips.
...
This installs `devlaunch` along with `devpod` and all dependencies.
...
With pip you must install [devpod](https://devpod.sh/docs/getting-started/install) separately.
...
what devpod said on stderr and exits `1` rather than reporting that you have no workspaces. That way `dl --purge`
...
off. The suffix is there to keep two branches from sharing an id, and reading it is no
part of choosing a workspace. The owner is not in the id at all, so a fork and its
...
picked, and two rows reading the same would be one workspace deleted in place of
...
Every verb in that table also takes the workspace second, `dl stop <user/repo>`. With no
...
`rm` is a word and `--rm` is a flag, and they are two different requests. That is docker's
...
own name, since the two binaries are built from one cargo package and cannot disagree
...
the prompt while it does. Type it free of shell quoting, with no escaping and no
...
quoting to survive `aid`'s own parsing. Managing workspaces, meaning listing,
stopping, deleting and VS Code, stays with `dl`.
